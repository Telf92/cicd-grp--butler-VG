...
def add_watched_url():
    ...
    try:
        ...
        try:
            activate_at = datetime.fromisoformat(data["activateAt"])
        ...
        # TODO: Ping if force is false and abort if the ping fails.
...
def delete_url(url_id):
    try: 
        persistance.delete_url(url_id)
    except persistance.ItemNotFoundError as e:
        return jsonify({'error': e.message }), 404

    return jsonify({'message': f'Removed watched url with id {url_id}'}), 200

@app.route('/watched-urls/<int:urlid>', methods=['GET'])
def get_url_data(urlid):
