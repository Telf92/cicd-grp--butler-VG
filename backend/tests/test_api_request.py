...
def test_set_watched_url():
    ...
    assert requests.post(API_WATCHED_URL, json=to_post, timeout=20).json() == {
...
def test_get_stats():
    assert requests.get(API_STATS_URL, timeout=20).json() == {
        "pings": 1,
        "watchedUrls": 1,
    }

def test_delete_url():
    assert requests.delete(API_DELETE_ULR, timeout=20).json() == {
